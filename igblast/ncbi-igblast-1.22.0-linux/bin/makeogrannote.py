...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Script to build V gene FWR/CDR annotation file from OGRDB germline sets JSON files"
    )
    parser.add_argument(
        "--germline_file",
        required=True,
    ...
    parser.add_argument(
        "--output_file", required=True, help="output annotation file name"
    )
            ...
            if each_allele["sequence_type"] == "V":
                ...
                for j in each_allele["v_gene_delineations"]:
                    if j["delineation_scheme"].lower() == annotation_type[0]:
                        ...
                        cdr1_start = j["cdr1_start"] - GetDotCounts(
                            seq, j["cdr1_start"]
                        )
                        ...
                        fwr2_start = j["fwr2_start"] - GetDotCounts(
                            seq, j["fwr2_start"]
                        )
                        ...
                        cdr2_start = j["cdr2_start"] - GetDotCounts(
                            seq, j["cdr2_start"]
                        )
                        ...
                        fwr3_start = j["fwr3_start"] - GetDotCounts(
                            seq, j["fwr3_start"]
                        )
                        outfile_handle.write(str(fwr3_start) + "\t")

                        fwr3_end = j["fwr3_end"] - GetDotCounts(
                            seq, min(len(seq), j["fwr3_end"])
                        )
                        outfile_handle.write(str(fwr3_end) + "\t")

                        chain_type = (
                            each_allele["sequence_type"]
                            + each_allele["locus"][len(each_allele["locus"]) - 1]
                        )
